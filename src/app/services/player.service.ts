...
export class PlayerService {
  ...
  constructor(
    private musicKitService: MusicService
  ) {
  ...
  setQueueFromItems( items: any[], startIndex: number = 0 ): Observable<any> {
    items.forEach( item => item.container = { id: item.id } );
    return from( this.musicKitService.musicKit.setQueue( { items, startPosition: startIndex - 1 } ) )
      .pipe( mergeMap( x => this.play() ) );
  }

  setQueue( items: any[], startIndex: number = 0 ): Observable<any> {
    items.forEach( item => item.container = { id: item.id } );
    return from( this.musicKitService.musicKit.setQueue( { items, startPosition: startIndex - 1 } ) );
  }
